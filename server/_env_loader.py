"""
Standalone environment configuration loader for Mem0 services.

This file is intentionally dependency-free so it can be copied into each
service directory (server/, openmemory/api/) and used before any other
imports — including before the mem0 package itself is installed.

Canonical source: mem0/configs/env_loader.py in the repository root.
If you modify this file, also update the canonical source and the other
copies in openmemory/api/ so they stay in sync.
"""
...
class ConfigValidationError(Exception):
    def __init__(self, errors: List[ConfigError]):
        ...
        lines = ["\n" + "=" * 72]
        lines.append(
            "  CONFIGURATION ERROR — the following environment variables are missing or invalid:"
        )
        lines.append("=" * 72)
        for err in errors:
            lines.append(f"\n  ❌ {err.key}")
    ...
    if not password:
        errors.append(
            ConfigError(
                ...
                suggestion=(
                    "Set POSTGRES_PASSWORD in your .env file. In docker-compose this is also required "
                    "for the postgres service to start."
                ),
            )
        )
        ...
        raise ConfigValidationError(
            [
                ConfigError(
                    ...
                    suggestion=(
                        "Set POSTGRES_PASSWORD in your .env file or pass password= explicitly."
                    ),
                )
            ]
        )
